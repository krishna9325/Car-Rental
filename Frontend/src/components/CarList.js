...
function CarList({
  actionType = "view", // "view", "edit", "admin"
  onCarAction,
  onSecondaryAction,
  showAlert = true,
}) {
  const { cities, loading, error, getCarsByCity } = useCars();
  ...
  // Update filtered cars when cities change or city selection changes
  useEffect(() => {
    if (cities.length > 0) {
      const cars = getCarsByCity(selectedCity);
      setFilteredCars(cars);
    }
  }, [cities, selectedCity, getCarsByCity]);

  const handleCityChange = (e) => {
    setSelectedCity(e.target.value);
  };

  // Action button configuration based on type
  const getActionConfig = () => {
    switch (actionType) {
      case "admin":
        return {
          primary: { text: "Edit", color: colors.button.primary },
          secondary: null,
        };
      case "view":
        return {
          primary: { text: "View", color: colors.button.primary },
          secondary: null, //Null for now. Add delete button later
        };
      case "Home":
      default:
        return {
                  ...
                  <Card.Body className="p-4">
                    ...
                    {actionConfig.primary && (
                      <div className="d-flex gap-2">
                        <Button
                          size="sm"
                          onClick={() => onCarAction && onCarAction(car.id)}
                          style={{
                            background: actionConfig.primary.color,
                            border: "none",
                            borderRadius: "10px",
                            padding: "8px 16px",
                            fontSize: "0.95rem",
                            flex: actionConfig.secondary ? "1" : "1",
                          }}
                          className="hover-scale"
                        >
                          {actionConfig.primary.text}
                        </Button>
                        ...
                        {actionConfig.secondary && (
                          <Button
                            size="sm"
                            onClick={() =>
                              onSecondaryAction && onSecondaryAction(car.id)
                            }
                            style={{
                              background: actionConfig.secondary.color,
                              border: "none",
                              borderRadius: "10px",
                              padding: "8px 16px",
                              fontSize: "0.95rem",
